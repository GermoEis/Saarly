...
export default function FloatingScreen() {
  ...
  const items = sortItems(app.state.items.filter((item) => !item.assigned_to && item.status === 'unassigned' && !app.state.lists.find((list) => list.id === item.list_id)?.archived_at), sortOrder);
