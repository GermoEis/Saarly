...
export default function ListsScreen() {
  ...
  const remove = (id: string, listName: string) => { const run = () => app.deleteList(id); const message = `Kas kustutada nimekiri „${listName}“? Juba jooksvasse listi vabastatud tooted jäävad alles.`; if (Platform.OS === 'web') { if (window.confirm(message)) run(); } else Alert.alert('Kustuta nimekiri?', message, [{ text: 'Loobu', style: 'cancel' }, { text: 'Kustuta', style: 'destructive', onPress: run }]); };
...
const makeStyles = (colors: ThemeColors) => StyleSheet.create({ grid: { width: '100%', minWidth: 0, flexDirection: 'row', flexWrap: 'wrap', gap: 16 }, listWrap: { minWidth: 0, maxWidth: '100%', flexBasis: 400, flexGrow: 1, flexShrink: 1 }, listWrapNarrow: { width: '100%', flexBasis: '100%' }, listHead: { minWidth: 0, flexDirection: 'row', alignItems: 'flex-start', gap: 10 }, listName: { fontSize: 21, lineHeight: 27, fontWeight: '800', letterSpacing: -.3, color: colors.ink }, desc: { color: colors.muted, fontSize: 15, lineHeight: 22, marginTop: 4 }, chevronWrap: { width: 36, height: 36, borderRadius: 18, backgroundColor: colors.primarySoft, alignItems: 'center', justifyContent: 'center', flexShrink: 0 }, progressTrack: { height: 7, borderRadius: 99, backgroundColor: colors.progressTrack, overflow: 'hidden' }, progress: { height: '100%', backgroundColor: colors.primary, borderRadius: 99 }, progressText: { color: colors.muted, fontSize: 14, fontWeight: '700' }, delivery: { minWidth: 0, backgroundColor: colors.accentSoft, borderWidth: 1, borderColor: colors.accentBorder, borderRadius: 13, padding: 12, flexDirection: 'row', alignItems: 'center', gap: 11 }, deliveryIcon: { width: 38, height: 38, borderRadius: 19, backgroundColor: colors.surface, alignItems: 'center', justifyContent: 'center' }, deliveryTitle: { color: colors.ink, fontSize: 15, fontWeight: '800' }, formError: { fontSize: 16, lineHeight: 22 }, settlementCard: { flexDirection: 'row', alignItems: 'center', gap: 14 }, settlementIcon: { width: 50, height: 50, borderRadius: 15, backgroundColor: colors.primarySoft, borderWidth: 1, borderColor: colors.secondaryBorder, alignItems: 'center', justifyContent: 'center' }, settlementIconText: { color: colors.primaryDark, fontSize: 25, fontWeight: '900' } });
