    ...
    <Pressable accessibilityRole="button" accessibilityLabel={`Ava toode ${item.name}`} onPress={() => setOpen(true)}>
      <Card style={compact ? styles.compact : undefined}>
        <View style={styles.topRow}>
          ...
          {image ? <View style={styles.photoMark}><Text style={styles.photoIcon}>▣</Text></View> : null}
        </View>
        <View style={styles.metaRow}><StatusBadge status={item.status} />{assigned ? <Text style={styles.assigned}>👤 {assigned.display_name}</Text> : null}</View>
...
const makeStyles = (colors: ThemeColors) => StyleSheet.create({
  compact: { padding: 15 }, topRow: { flexDirection: 'row', gap: 10, alignItems: 'center' }, name: { fontSize: 18, fontWeight: '800', color: colors.ink }, quantity: { fontSize: 16, color: colors.muted }, metaRow: { flexDirection: 'row', gap: 10, alignItems: 'center', flexWrap: 'wrap' }, assigned: { color: colors.muted, fontSize: 14, fontWeight: '700' }, warning: { color: colors.warning, fontWeight: '800', fontSize: 14 }, note: { color: colors.muted, fontSize: 16, lineHeight: 23 }, photoMark: { width: 38, height: 38, borderRadius: 9, backgroundColor: colors.primarySoft, alignItems: 'center', justifyContent: 'center' }, photoIcon: { color: colors.ink, fontSize: 17 }, detailLead: { flexDirection: 'row', justifyContent: 'space-between', alignItems: 'center', gap: 10 }, bigQuantity: { fontSize: 20, fontWeight: '800', color: colors.ink }, infoBox: { backgroundColor: colors.subtle, padding: 14, borderRadius: 12, gap: 9 }, infoTitle: { color: colors.ink, fontWeight: '800', fontSize: 14 }, actions: { gap: 10 }, chips: { flexDirection: 'row', flexWrap: 'wrap', gap: 7 }, chip: { borderWidth: 1, borderColor: colors.fieldBorder, backgroundColor: colors.field, borderRadius: 99, paddingHorizontal: 11, paddingVertical: 8 }, chipActive: { backgroundColor: colors.primary, borderColor: colors.primary }, chipText: { color: colors.ink, fontWeight: '700' }, chipTextActive: { color: colors.onPrimary }, imageWrap: { height: 220, overflow: 'hidden', borderRadius: 14, backgroundColor: colors.photoSurface }, image: { width: '100%', height: '100%' }, photoEmpty: { height: 120, borderRadius: 14, borderWidth: 1, borderStyle: 'dashed', borderColor: colors.fieldBorder, alignItems: 'center', justifyContent: 'center', gap: 6 }, photoEmptyIcon: { color: colors.ink, fontSize: 28 }, sectionTitle: { fontSize: 18, fontWeight: '800', color: colors.ink }, timeline: { flexDirection: 'row', gap: 10, paddingBottom: 6 }, dot: { width: 10, height: 10, borderRadius: 5, backgroundColor: colors.primary, marginTop: 7 }, timelineText: { fontSize: 15, lineHeight: 22, color: colors.ink }, timelineNote: { color: colors.muted, marginTop: 2 },
});
