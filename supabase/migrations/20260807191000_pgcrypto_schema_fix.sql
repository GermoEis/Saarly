-- Supabase paigaldab pgcrypto funktsioonid extensions skeemi; security definer RPC-del on tühi search_path.
create or replace function public.set_group_security_code(target_group uuid, new_code text) returns void
...
revoke all on function public.set_group_security_code(uuid,text), public.redeem_group_code(text,text) from public;
grant execute on function public.set_group_security_code(uuid,text), public.redeem_group_code(text,text) to authenticated;
